...
ItemCreationWidget::ItemCreationWidget(QWidget *parent)
...
ItemCreationWidget::~ItemCreationWidget()
{
}

void ItemCreationWidget::setupUI()
{
    QVBoxLayout *main = new QVBoxLayout(this);

    QLabel *title = new QLabel(tr("Create Item (using template 47+)"));
    ...
    _createButton = new QPushButton(tr("Create Item")); _createButton->setDefault(true);
        ...
        if (db.open()) {
            ...
            if (q.exec("SELECT code, name FROM items WHERE name LIKE '%Mystic Orb%' OR name LIKE '%Mystic%' OR tags LIKE '%myst%';")) {
                while (q.next()) {
                    ...
                    if (!code.isEmpty()) {
                        _itemCombo->addItem(code + " - " + name, code);
                    }
                }
                if (_itemCombo->count() > 0) loaded = true;
            }
            db.close();
        }
    ...
    if (!loaded) {
        ...
        if (f.exists() && f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            while (!in.atEnd()) {
                ...
                if (line.contains(QLatin1String(""), Qt::CaseInsensitive) || line.contains(QLatin1String(""))) {
...
void ItemCreationWidget::onCreateClicked()
{
    _copies = _copiesSpin->value();

    ItemInfo *orb = createItemItem();
    if (!orb) {
        QMessageBox::critical(this, tr("Error"), tr("Failed to create Item from template."));
        return;
    }
    _createdOrb = orb;
    accept();
...
void ItemCreationWidget::onCancelClicked()
{
    reject();
}

ItemInfo* ItemCreationWidget::createItemItem()
{
    // Determine selected orb code (prefer explicit selection)
    QString selectedCode;
    if (_itemCombo && _itemCombo->currentIndex() >= 0)
        selectedCode = _itemCombo->currentData().toString();
